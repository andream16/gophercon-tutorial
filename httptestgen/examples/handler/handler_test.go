...
func TestCreateUserHandler(t *testing.T) {
	...
	t.Run("it_should_succeed_when_a_valid_user_is_passed", func(t *testing.T) {
		...
		// Initialize request struct with concrete fields
		...
		// Use raw JSON comparison (no type mapping available)
	...
	t.Run("it_should_return_a_bad_request_when_the_request_is_invalid", func(t *testing.T) {
		...
		// Initialize expected response struct with concrete fields
		...
		if expectedResponse.Error != actualResponse.Error {
			t.Errorf("CreateUserHandler field Error mismatch:\ngot:  %q\nwant: %q", actualResponse.Error, expectedResponse.Error)
		}
		if expectedResponse.Code != actualResponse.Code {
			t.Errorf("CreateUserHandler field Code mismatch:\ngot:  %q\nwant: %q", actualResponse.Code, expectedResponse.Code)
		}
	})
}
func TestGetUserHandler(t *testing.T) {
	...
	t.Run("it_should_return_not_found_when_user_does_not_exist", func(t *testing.T) {
		...
		// Compare field by field for better error messages
		if expectedResponse.Error != actualResponse.Error {
			t.Errorf("GetUserHandler field Error mismatch:\ngot:  %q\nwant: %q", actualResponse.Error, expectedResponse.Error)
		}
		if expectedResponse.Code != actualResponse.Code {
			t.Errorf("GetUserHandler field Code mismatch:\ngot:  %q\nwant: %q", actualResponse.Code, expectedResponse.Code)
		}
	})
}
